...
Cette application Spring Boot Kotlin a pour finalité de mettre à disposition un service web de type API REST, au service d'applications frontend Angular réalisées par des étudiants développeurs d'applications, en mode projet.

== Analyse de la structure JSON initiale

Nous partons d'une liste de _boxes_ initiale au format JSON. Ce fichier est localisé à la racine de l'application et se nomme link:boxes-sushi.json[boxes-sushi.json].

L'objectif est de concevoir une API Rest qui produise ce résultat.
...
. Placer une copie du fichier d'initialisation json dans ce dossier (boxes-sushi.json).
...
TIP: Le serveur et application s'arrêtent avec un `CTRL+C` dans le terminal qui a lancé la commande comme signalé précédemment (sinon il faut tuer le processus)
